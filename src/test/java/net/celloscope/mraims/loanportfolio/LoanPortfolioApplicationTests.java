...
@SpringBootTest
@ActiveProfiles("dev")
class LoanPortfolioApplicationTests {
...
//        Mono<List<FeeCollection>> updatedFeeCollectionsMono = feeCollectionUseCase.updateNullableFeeCollectionByOfficeId
...
/*        updatedFeeCollectionsMono
                .flatMap(updatedCollections -> autoVoucherUseCase.createAndSaveAutoVoucherForFeeCollection(buildAutoVoucherRequest(updatedCollections)))
                .subscribe(createdAutoVoucher -> {
                    assertThat(createdAutoVoucher.getVoucherId()).isNotBlank();
                });*/
